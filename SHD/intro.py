...
plt.plot(np.ones(700),np.arange(0,700),'r--',linewidth=2)
plt.title("Label %i"%labels[k])
plt.xlabel('time [s]',fontsize=14)
plt.ylabel('Channel',fontsize=14)
plt.xlim([0,max(times[k])])
plt.ylim([0,700])
# plt.axis("off")
plt.show()

# test_X,testy = generate_dataset(files[0],dt=4e-3)
# np.save('/Volumes/Data/b_yin/SHD/data_coount/testX_4ms.npy',test_X)
# np.save('/Volumes/Data/b_yin/SHD/data_coount/testY_4ms.npy',testy)

# train_X,trainy = generate_dataset(files[1],dt=4e-3)
# np.save('/Volumes/Data/b_yin/SHD/data_coount/trainX_4ms.npy',train_X)
# np.save('/Volumes/Data/b_yin/SHD/data_coount/trainY_4ms.npy',trainy)
...
def binary_image_readout(times,units,dt = 1e-3):
    ...
    return np.array(img)
idx = k
tmp = binary_image_readout(times[idx],units[idx],dt=4e-3)
plt.figure(figsize=(10,5))
plt.imshow(tmp.T,interpolation='nearest', aspect='auto',origin='lower',extent=[0,1.,0,700])
plt.show()

plt.figure(figsize=(10,3))
plt.plot(np.arange(0,1,4e-3),np.sum(tmp,axis=1))
plt.xlim([0.,1.])
